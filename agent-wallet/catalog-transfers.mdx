...
USDC `submit_agent_wallet_transfer` and `paybox_request_transfer` use the same
signing step. Neither returns a signing plan or approval URL to a model. Those
stay browser-only.
...
- Through MCP, a pending request carries `signing_handoff.console_url`. Paste it
  ...
  Never ask them to paste a key or signature into chat. If `needs_mailbox` is
  true, call `get_agent_wallet` with `mailboxId` first.
