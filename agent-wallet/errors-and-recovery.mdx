...
- A pending swap is not success. Use its PayBox MCP App or one returned signing
  handoff, then call `paybox_get_request` once only after the user confirms
  signing or asks for status.
- Never replace a pending or uncertain x402 payment. The authenticated browser
  continuation may poll the same request and reopen its signing window once;
  the model must not call `paybox_pay_x402` again.
