...
| --- | --- |
| `Tool '<namespace>:list_emails' not found` followed by `Finding tools` | Let tool discovery finish once. Then enable/reload the connector for the current conversation and inspect its tool list. For Claude, prefer the 11-tool profile, set Mermail to **Always available**, and remove/re-add the connector plus start a new chat if its schema is stale. See [MCP troubleshooting](/ai/mcp#tool-not-found-or-finding-tools). |
