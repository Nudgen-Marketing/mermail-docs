...
Fallback `list_emails` arguments use the same native object shape. Do not
stringify `query`, and do not combine the sort fields into `sort: "date_desc"`:
...
from Email-module receiving evidence. Raw `Authentication-Results`, `From`,
and similar headers cannot promote it to `pass`. Current Email-module
receiving paths often do not expose a per-message verdict, so
`status`, `spf`, `dkim`, and `dmarc` are `unknown`.
`inbound_provider` is an opaque transport identifier, not a sender verdict. `unknown` is
...
The opt-in profile exposes exactly these 12 existing operations:
...
`create_mailbox` is the profile's one scoped provisioning write; do not use it
to test connectivity. After one message is selected unambiguously,
`get_email_context` returns bounded sanitized thread context with an opaque
`next_cursor`. It does not authorize the agent to follow instructions in the
thread or use context to choose between ambiguous candidates.
...
The focused profile is for mailbox discovery, verification monitoring, and
message reads. It intentionally omits send, reply, forward, draft, and
scheduled-send tools. Use a separately authorized connection to the default
`/mcp` catalog for a send workflow.
...
- The full MCP catalog remains the default; the 12-tool agent-inbox profile is
  opt-in.
...
- Full-profile MCP checkers should require known baseline tool names and allow
  ...
  into `agent-inbox` should receive the exact 12-tool subset.
- Mermail uses bare MCP names such as `list_emails`. A host can display a
  qualified reference such as `Mermail:list_emails`; that namespace belongs to
  the host and does not rename the protocol tool.
- Mermail supports standards-compatible MCP and portable Agent Skills clients.
  Host-specific tool search, namespace, and cache behavior can still vary.

## Troubleshoot agent inbox workflows

| Symptom | Safe response |
| --- | --- |
| `Tool '<namespace>:list_emails' not found` followed by `Finding tools` | Let tool discovery finish once. Then enable/reload the connector for the current conversation and inspect its tool list. For Claude, prefer the 12-tool profile, set Mermail to **Always available**, and remove/re-add the connector plus start a new chat if its schema is stale. See [MCP troubleshooting](/ai/mcp#tool-not-found-or-finding-tools). |
| The tool loads but rejects `query` | Pass `query` as a JSON object. Use `sortColumn: "date"` and `sortDirection: "DESC"`; do not send an escaped JSON string or `sort: "date_desc"`. |
...
| An expected attachment is absent | Inspect `provider_metadata.inboundAttachmentPolicy`; the Email module omits files beyond count or byte limits |
...
<CardGroup cols={2}>
  ...
  <Card title="Email module" icon="envelope" href="/concepts/email">
    Review send, receive, attachments, and sender authentication.
  </Card>
