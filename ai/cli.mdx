...
Flags, enum choices, required query parameters, and request validation are generated from the checked-in OpenAPI contract for each operation. Unrelated flags are not shown. Nested or complete bodies can still use JSON, a file, or stdin:
...
| `explore` | Interactive traversal in a human-operated terminal |

```bash
mermail mailboxes list --transform '[].email'
mermail mailboxes list --format explore
...
`--transform` uses JMESPath and runs before formatting. Agents and CI should keep the default JSON format; `explore` requires an interactive terminal.
...
The CLI validates all 62 method/path pairs against a reproducible OpenAPI snapshot. A scheduled remote check compares the same 62 operation names with the production MCP server card so additions and removals cannot silently drift.

Only read requests retry transient network failures, `408`, `429`, `502`, `503`, and `504`. `Retry-After` supports both seconds and HTTP dates and is capped at 30 seconds. Writes and external effects are never retried automatically.
