...
- Wallet commands ignore `MERMAIL_API_KEY` and require a prior `auth login` (except `fund-url`, which only prints a console deep link).
...
- `wallet status` can report `connection.status: "PAYBOX_UNAVAILABLE"` with an empty portfolio when PayBox did not answer that read. The connection is still active; read again later.
