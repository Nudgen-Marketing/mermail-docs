...
<AccordionGroup>
  ...
  <Accordion title="Agent Wallet (OAuth + wallet scopes only)">
    `get_agent_wallet`, `list_agent_wallet_credentials`, `get_agent_wallet_portfolio`, `get_agent_wallet_request`, `create_agent_wallet_transfer_proposal`, `submit_agent_wallet_transfer`, `reject_agent_wallet_transfer_proposal`, `get_paybox_invocation`, plus reviewed `paybox_*` tools when PayBox is connected
