...
MCP clients that support OAuth discover Mermail via Protected Resource Metadata, open the console authorize page, and receive a Bearer access token after the user signs in with their Mermail account (same as the web app) and picks a workspace. The consent UI shows a friendly client name (for example ChatGPT or Cursor), not the opaque `mcp_client_…` id.
...
For packaged workflows, install [Mermail Skills](/ai/skills) (`npx --yes skills add Nudgen-Marketing/mermail-skills`) or connect via the registry id `app.mermail/mcp` when your host supports Official Registry install.
...
The five email write tools in the table above accept `attachments`. Each item is either base64 file bytes or an existing attachment reference from the same authorized mailbox. The server checks mailbox access and resolves reference metadata and content; a missing, deleted, or foreign reference fails the request.
...
Replace the example `content` with the base64 encoding of the complete file. A local path, URL, or `data:` URL is not file content. MCP tools cannot open a path on your computer. Existing flat arguments remain supported, but use the discoverable nested `body` schema for new integrations.

When you select an existing draft with `source_draft_id` for send/reply/forward or `draft_id` for save/schedule, omitting `attachments` preserves its files. An explicit array replaces the complete set; `[]` removes all attachments. Include `{ "attachment_id": "..." }` entries for any existing files you want to keep when adding or removing others.

For an inline image, use `disposition: "inline"`, a unique bare `contentId` such as `chart-1`, and HTML containing `<img src="cid:chart-1" alt="Chart" />`. Inline PNG, JPEG, GIF, and WebP bytes must match their MIME type. Other file types can be ordinary attachments subject to provider restrictions. Drafts use the same HTML in their string `body` with `body_format: "html"`.

Storage limits are **20 files**, **10 MiB per file**, and **25 MiB decoded total**. Delivery also checks the encoded message, its bodies, and overhead: hosted `@mermail.app` addresses use about **5 MiB**, while custom-domain addresses allow about **40 MB**. The server reserves overhead conservatively, so a file below its storage limit can still be too large to send. See [Email module limits](/concepts/email#attachments-and-message-size).

MCP request bodies have a **40 MiB streamed ceiling**, including JSON and base64 encoding. Oversized requests return HTTP `413` with `request_payload_too_large`. File and metadata validation returns field details; provider message-size rejection uses `outbound_message_too_large`.

Report a successful draft save as **drafted**, a schedule as **scheduled**, and a queued send as **queued**. Report **sent** only after an authoritative send result confirms it; this does not prove recipient delivery. If the outcome is uncertain, inspect the existing message's status before retrying to avoid duplicate emails.

Server instructions: prefer read-only tools before writes. Responses are JSON text plus object-shaped `structuredContent`. JSON arrays are exposed as `{ "items": [...] }` so the result conforms to the MCP schema. Binary **download responses** are capped at **1 MiB**; this is separate from the outbound limits above. Report that download limit instead of inventing a storage URL; use the authenticated REST attachment endpoint when your authorized workflow requires a larger download.
...
`inbound_provider`, and `reason`. Mermail derives it only from Email-module
receiving evidence, never from raw `Authentication-Results`, `From`, or
other message headers. Current Email-module receiving paths often return
`unknown`. `unknown` is not a pass, and `inbound_provider` is an opaque
transport identifier rather than a sender verdict. Even a future
...
2. Receive `{ confirmationToken, expiresInSeconds }` (token prefix `mcp_confirm_`, TTL **5 minutes**, single-use).
...
<Warning>
  If confirmation is unavailable, `prepare_destructive_action` fails with `503` `confirmation_unavailable`.
</Warning>
...
| `503` `confirmation_unavailable` | Destructive-action confirmation is unavailable. Do not call the destructive tool; wait for the service to recover and prepare a new confirmation. |
