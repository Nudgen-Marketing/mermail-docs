...
Exact config keys differ by host (Cursor, Claude Code, Codex, custom MCP clients). The important parts are the `/mcp` URL, Streamable HTTP (not SSE), and `x-api-key`.
