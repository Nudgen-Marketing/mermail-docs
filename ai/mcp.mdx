...
Agent Wallet and live PayBox tools appear only on the default full-profile
**OAuth** session for the **workspace owner**. API-key catalogs and the
`agent-inbox` profile never include them.

| Scope | Purpose |
| --- | --- |
| `mcp:tools` | Core Mermail tool access. For workspace owners on the full profile, this also exposes Agent Wallet and eligible live PayBox tools. |
...
Connect to the default `/mcp` endpoint with OAuth, select the intended workspace
as its owner, then use `get_paybox_connection` or `get_agent_wallet`. If they
return `connect_handoff` or `reauth_handoff`, open that Mermail Agent Wallet
`console_url` and select **Connect** / reconnect. Do not add legacy wallet
scopes or reconnect Claude, ChatGPT, or Codex connector settings to authorize
PayBox.
...
**OAuth** sessions with core `mcp:tools` for the workspace owner can expose
additional Agent Wallet / PayBox tools on top of that baseline. Grouped by area:
...
<AccordionGroup>
  ...
  <Accordion title="Agent Wallet (full-profile owner OAuth only)">
    ...
    Not available to API keys or the agent-inbox profile. Requires the core `mcp:tools` grant on a full-profile OAuth session as workspace owner; legacy `wallet:read` / `wallet:transact` labels are compatibility-only. Checkout / MoonPay URLs stay browser-only (`[redacted]`); use `funding_handoff.console_url` for Funding, `signing_handoff.console_url` for pending transfers, and `connect_handoff` / `reauth_handoff` for PayBox Connect or reconnect inside Mermail — never host connector settings. Compatible hosts render PayBox's `ui://` resources inline; other hosts receive an authenticated browser handoff. Secret credentials and signing plans stay browser-only. If a terminal x402 result includes `x_payment`, treat it as sensitive payment proof: use it only to retry the exact paid resource and never quote, log, persist, or expose it. See [Agent Wallet](/agent-wallet/overview).

    Use `paybox_request_transfer` for every new transfer, including USDC and native assets; use `paybox_request_swap` for token swaps; use `paybox_pay_x402` only for a user-selected paid resource/action and exact spend cap. Read each live schema instead of reusing legacy proposal fields. See [catalog token transfers](/agent-wallet/catalog-transfers) and [swaps and x402](/agent-wallet/swaps-and-x402).
  </Accordion>
