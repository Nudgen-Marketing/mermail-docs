...
Default consent may omit wallet scopes. Reconnect OAuth, approve `wallet:read` / `wallet:transact`, then select **Connect** on the mailbox [Agent Wallet](/agent-wallet/get-started#connect-paybox) page in the console. Only the workspace owner can use Agent Wallet.
...
At publication time, an **API-key** session exposes **71** tools:
`prepare_destructive_action` plus **70** Sold API wrappers. **OAuth** sessions
...
<AccordionGroup>
  ...
  <Accordion title="Agent Wallet (OAuth + wallet scopes only)">
    ...
    Not available to API keys. Requires `wallet:read` and/or `wallet:transact`. Checkout / MoonPay URLs stay browser-only (`[redacted]`); use `funding_handoff.console_url` for Funding. See [Agent Wallet](/agent-wallet/overview).
