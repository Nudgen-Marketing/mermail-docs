...
Agent Wallet and live PayBox tools appear only on **OAuth** sessions. API-key catalogs never include them.

| Scope | Purpose |
| --- | --- |
| `wallet:read` | Agent Wallet status plus PayBox tools annotated upstream with `readOnlyHint: true` |
| `wallet:transact` | Every PayBox tool that is not explicitly annotated read-only, including transaction and signing flows |

Default consent may omit wallet scopes. Reconnect OAuth, approve `wallet:read` / `wallet:transact`, then use `get_paybox_connection` / `get_agent_wallet`. If they return `connect_handoff` or `reauth_handoff`, open that Mermail Agent Wallet `console_url` and select **Connect** / reconnect — do not reconnect Claude, ChatGPT, or Codex connector settings for PayBox. Only the workspace owner can use Agent Wallet.
...
the PayBox MCP App handles it. Pending or `SUBMISSION_UNKNOWN` is not success.
...
Destructive Mermail tools (delete workspace/member/domain/email/folder/label/conversation/triager, bulk delete, empty trash, …) require a short-lived confirmation token:
...
to PayBox after Mermail verifies workspace ownership and the required wallet
scope.
...
<AccordionGroup>
  ...
  <Accordion title="Agent Wallet (OAuth + wallet scopes only)">
    `get_paybox_connection`, `get_agent_wallet`, `list_agent_wallet_credentials`, `get_agent_wallet_portfolio`, `get_agent_wallet_request`, `get_paybox_invocation`, plus the live `paybox_*` catalog when PayBox is connected. Legacy proposal/submit/reject names remain deprecated compatibility aliases.

    Not available to API keys. Requires `wallet:read` and/or `wallet:transact` (`get_paybox_connection` may appear on OAuth even before wallet scopes and then return `SCOPE_UPGRADE_REQUIRED`). Checkout / MoonPay URLs stay browser-only (`[redacted]`); use `funding_handoff.console_url` for Funding, `signing_handoff.console_url` for pending transfers, and `connect_handoff` / `reauth_handoff` for PayBox Connect or reconnect inside Mermail — never host connector settings. Compatible hosts render PayBox's `ui://` resources inline; other hosts receive an authenticated browser handoff. Secret credentials and signed payloads are never model-visible. See [Agent Wallet](/agent-wallet/overview).

    `paybox_request_transfer` takes `chain`, `token` (a portfolio contract address, or `native`), `to`, and the human amount in `amount_decimal`. Mermail converts to base units and rejects an integer `amount` for any asset it can price. See [catalog token transfers](/agent-wallet/catalog-transfers).
  </Accordion>
