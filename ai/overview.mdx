...
<CardGroup cols={2}>
  ...
  <Card title="Agent Wallet" icon="wallet" href="/agent-wallet/overview">
    View delegated balances and use PayBox's live tools and signing UI.
  </Card>
</CardGroup>
...
| View balances or use delegated PayBox capabilities         | [Agent Wallet](/agent-wallet/overview)     | Full-profile MCP OAuth; owner manages connection     |
...
<Steps>
  <Step title="Connect the least-privilege MCP profile">
    ...
    the exact 12 tools needed for mailbox discovery, one optional provision,
    bounded email search, one message read, and bounded thread context.
  </Step>
...
Current Email-module receiving paths can return `sender_authentication.status:
