...
<Accordion title="Can I create an address on the mermail.app domain?">
  ...
  <Accordion title="Can I use my own domain?">
    Yes, on a paid plan. Each workspace can connect one custom domain. Use a
    dedicated subdomain such as `mail.example.com` so you do not disrupt email
    delivery for your main domain. After you publish the DNS records and the
    connection is verified, every mailbox in the workspace moves to the domain.
    See [Custom domains](/guides/custom-domains).
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="Where are message bodies and attachments stored?">
    Standard hosted workspaces store full message bodies and attachments in
    managed encrypted object storage instead of plain database text. Storage access is
...
<Accordion title="How do plan limits work?">
  ...
  <Accordion title="How can I get help?">
    ...
    [Mermail Discord community](https://discord.gg/ZNQWVxrErk). See
